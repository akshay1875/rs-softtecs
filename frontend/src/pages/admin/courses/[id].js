...
export default function EditCoursePage() {
  ...
  const fetchCourse = async () => {
    try {
      ...
      setFormData({
        ...
        icon: course.icon || 'fas fa-laptop-code',
        status: course.status || 'active',
        isFeatured: course.isFeatured || false,
        syllabus: course.syllabus?.length > 0
          ? course.syllabus.map(m => ({ title: m.title || '', topics: m.topics?.length > 0 ? m.topics : [''] }))
          : [{ title: '', topics: [''] }],
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await coursesAPI.update(id, cleanedData);
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="space-y-3">
              ...
              >
                {loading ? 'Saving...' : <><FaSave /> Update Course</>}
              </button>
              <button
                type="button"
                onClick={() => router.back()}
